...
class Client
{
    ...
    public function __construct($config = [])
    {
        ...
        $handlerStack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return TetherdResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    public function request($method, $params = [])
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\TetherdException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\TetherdException($exception->getResponse()->error());
            }
        ...
        } catch (Exceptions\TetherdException $exception) {
            throw $exception;
        }
    }

    /**
     * Makes async request to tether Core.
     *
     * @param string        $method
     * @param mixed         $params
        ...
        callable $onRejected = null)
    {
        ...
        $promise->then(
            function (ResponseInterface $response) use ($onFullfiled) {
                $error = null;
                if ($response->hasError()) {
                    $error = new Exceptions\TetherdException($response->error());
                }
            ...
            function (RequestException $exception) use ($onRejected) {
                if (
                    ...
                    $exception = new Exceptions\TetherdException(
    ...
    /**
     * Converts amount from satoshi to tether.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from tether to satoshi.
     *
     * @param float $amount
     *
     * @return int
